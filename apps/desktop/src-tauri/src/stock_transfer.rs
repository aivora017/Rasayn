...
pub fn stock_transfer_list(
    ...
    if shop_id.is_some() {
        sql.push_str(" AND (from_shop_id = ?1 OR to_shop_id = ?1)");
    }
    if status.is_some() {
        sql.push_str(if shop_id.is_some() {
            " AND status = ?2"
        } else {
            " AND status = ?1"
        });
...
pub fn stock_transfer_dispatch(
    ...
    }
    {
        ...
        let rows: Vec<(String, String, String, f64)> = stmt
            .query_map(params![transfer_id], |r| {
...
pub fn stock_transfer_receive(
    ...
    for ln in &input.lines {
        ...
        let Some((product_id, batch_id)) = row else {
            continue;
        };
...
pub fn stock_transfer_list_lines(
    ...
    let collected: Vec<StockTransferLine> = stmt
        .query_map(params![transfer_id], |r| {
