...
pub fn shops_list(state: State<'_, DbState>) -> Result<Vec<ShopRow>, String> {
    ...
    let mut stmt = c
        .prepare("SELECT id, name FROM shops ORDER BY name")
        .map_err(|e| e.to_string())?;
    let rows: Vec<ShopRow> = stmt
        .query_map([], |r| {
            Ok(ShopRow {
                id: r.get(0)?,
                name: r.get(1)?,
...
pub fn batches_list_by_shop(
    ...
    let mut stmt = c
        ...
        .map_err(|e| e.to_string())?;
    let rows: Vec<ShopStockRow> = stmt
        .query_map(params![shop_id, lim], |r| {
            Ok(ShopStockRow {
                shop_id: r.get(0)?,
                shop_name: r.get(1)?,
                product_id: r.get(2)?,
...
pub fn shops_inventory_summary(state: State<'_, DbState>) -> Result<Vec<ShopSummaryRow>, String> {
    ...
    let mut stmt = c
        ...
        .map_err(|e| e.to_string())?;
    let rows: Vec<ShopSummaryRow> = stmt
        .query_map([], |r| {
            Ok(ShopSummaryRow {
                shop_id: r.get(0)?,
                shop_name: r.get(1)?,
                product_count: r.get(2)?,
                batch_count: r.get(3)?,
                total_units: r.get(4)?,
            })
        })
