...
fn upsert_sensor_then_log_reading() {
    ...
    let n: i64 = c
        .query_row(
            "SELECT count(*) FROM cold_chain_readings WHERE sensor_id='s1'",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
fn excursion_open_then_close() {
    ...
    let id: i64 = c
        .query_row(
            "SELECT id FROM cold_chain_excursions WHERE sensor_id='s1'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    ...
    let end: Option<String> = c
        .query_row(
            "SELECT excursion_end FROM cold_chain_excursions WHERE id = ?1",
            params![id],
            |r| r.get(0),
        )
        .unwrap();
