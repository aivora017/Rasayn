...
describe("SettingsScreen (F5b)", () => {
  ...
  it("rejects malformed GSTIN without hitting backend", async () => {
    ...
    const gstin = screen.getByTestId("f-gstin");
    await user.clear(gstin);
    await user.type(gstin, "INVALIDGSTIN123");
    await user.click(screen.getByTestId("f-save"));
  ...
  it("requires state_code to match first 2 chars of GSTIN", async () => {
    ...
    await user.clear(screen.getByTestId("f-name"));
    await user.type(screen.getByTestId("f-name"), "Vaidyanath Pharmacy");
    await user.clear(screen.getByTestId("f-gstin"));
    await user.type(screen.getByTestId("f-gstin"), "27ABCDE1234F1Z5");
    await user.clear(screen.getByTestId("f-state"));
    await user.type(screen.getByTestId("f-state"), "29");
    await user.clear(screen.getByTestId("f-license"));
    await user.type(screen.getByTestId("f-license"), "21B/MH/KL/999");
    await user.clear(screen.getByTestId("f-address"));
    await user.type(screen.getByTestId("f-address"), "1st Floor, Kalyan");
  ...
  it("saves valid input and clears placeholder banner", async () => {
    ...
    await user.type(screen.getByTestId("f-gstin"), "27ABCDE1234F1Z5");
    await user.clear(screen.getByTestId("f-state"));
    await user.type(screen.getByTestId("f-state"), "27");
    await user.clear(screen.getByTestId("f-license"));
    await user.type(screen.getByTestId("f-license"), "21B/MH/KL/999");
    await user.clear(screen.getByTestId("f-address"));
    await user.type(screen.getByTestId("f-address"), "1st Floor, Kalyan");
  ...
  it("upper-cases GSTIN input automatically", async () => {
    ...
    expect(gstin.value).toBe("27ABCDE1234F1Z5");
  });
  ...
  it("Reset button reverts dirty edits to loaded values", async () => {
    ...
    await user.clear(screen.getByTestId("f-name"));
    await user.type(screen.getByTestId("f-name"), "Messed Up");
